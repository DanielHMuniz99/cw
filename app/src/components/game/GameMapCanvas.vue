...
function renderStaticMap() {
  ...
  offscreen.width = Math.max(1, Math.ceil(bounds.width + padding * 2))
  offscreen.height = Math.max(1, Math.ceil(bounds.height + padding * 2))
...
function renderFrame(timestamp: number) {
  ...
  if (staticMapCanvas.value) {
    const bounds = mapBounds.value
    const padding = 30
    ctx.drawImage(staticMapCanvas.value, bounds.minX - padding, bounds.minY - padding)
  } else {
    for (const province of props.provinces) {
      const path = makePolygonPath(province.points)
      ctx.fillStyle = province.color
